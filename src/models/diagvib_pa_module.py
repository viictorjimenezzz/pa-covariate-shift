    ...
    def on_train_epoch_end(self):
        if self.current_epoch == 0:
            ...
            for beta in self.betas:
                kernel = PosteriorAgreementKernel(beta=beta)        
                kernel.to(self.kernel.beta.device)
                kernel.eval()
                with torch.no_grad():
                    for o1,o2 in zip(self.training_step_outputs_1,self.training_step_outputs_2):
            ...
            logger.log({
                "all_betas": wandb.Table(data=self.betas),
                "all_PAs": wandb.Table(data=PAs),
                "all_log_PAs": wandb.Table(data=log_PAs)
            })
