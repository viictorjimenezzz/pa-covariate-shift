...
class CIFAR10DataModulePA(CIFAR10DataModule):
    """
    DataModule to use for PA optimization. 
    """
    def setup(self, stage: Optional[str] = None):
        super().setup(stage)
        self.train_ds = PosteriorAgreementDatasetPairing(self.train_ds)
    ...
    def train_dataloader(self):
        return DataLoader(
            ...
            num_workers=self.hparams.num_workers,
            pin_memory=self.hparams.pin_memory,
            shuffle=True, # We change shuffle to True, because we intend to optimize the PA.
        )


class CIFAR10DataModulePAlogits(LogitsPA, CIFAR10DataModulePA):
    def __init__(self, classifier: torch.nn.Module, *args, **kwargs):
        super().__init__(classifier)
        CIFAR10DataModulePA.__init__(self, classifier, *args, **kwargs)