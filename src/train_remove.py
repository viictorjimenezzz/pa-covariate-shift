...
def train(cfg: DictConfig) -> Tuple[dict, dict]:
    ...
    # ADD THE CONFIGURATION FOR THE PLOT -----------------------------------------------------------------------------
    import pickle

    OmegaConf.set_struct(cfg, False) # may it mutable
    ...
    if cfg.data.attack.attack_name == "PGD":
        cfg.model.beta_to_plot = df_betas[
            (df_betas["attack"] == cfg.data.attack.attack_name) & 
            (df_betas["defense"] == cfg.model.net.model_name) & 
            (df_betas["epsilon"] == str(cfg.auxiliary_args.epsilons))
        ]["beta"].item()
    elif cfg.data.attack.attack_name == "FMN":
        cfg.model.beta_to_plot = df_betas[
            (df_betas["attack"] == cfg.data.attack.attack_name) & 
            (df_betas["defense"] == cfg.model.net.model_name) 
        ]["beta"].item()
    ...
    OmegaConf.set_struct(cfg, True) # immutable again
    # -----------------------------------------------------------------------------------------------------------------
