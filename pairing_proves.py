...
# ds_train0 = DiagVib6DatasetPA(
#                 mnist_preprocessed_path = r"data/dg/mnist_processed.npz",
#                 cache_filepath = r"data/dg/dg_datasets/test_data_pipeline/train_singlevar0.pkl",
# )

# ds_train1 = DiagVib6DatasetPA(
#                 mnist_preprocessed_path = r"data/dg/mnist_processed.npz",
#                 cache_filepath = r"data/dg/dg_datasets/test_data_pipeline/train_singlevar1.pkl",
# )

# ds = MultienvDataset([ds_train0, ds_train1])

from src.models.components.dg_backbone import get_feature_extractor, DGBackbone
...
# TESTING WILDS
from pametric.datautils import MultienvDataset
from src.data.components.wilds_dataset import WILDSDatasetEnv, WILDS_multiple_to_single
from wilds import get_dataset
from src.data.components.wilds_transforms import initialize_transform
from omegaconf import OmegaConf
import yaml

celebA_yaml = r"configs/data/dg/wilds/celebA.yaml"
with open(celebA_yaml, 'r') as file:
    ...
    [
        WILDSDatasetEnv(
            ...
            transform = initialize_transform(
                dataset = OmegaConf.create({"original_resolution": [178, 218]}),
                transform_name = "image_base",
                config = OmegaConf.create({"target_resolution": [224, 224]}),
                is_training = True
            ),
            env_config = WILDS_multiple_to_single(OmegaConf.create(celebA_config["val_config"]))
        ),
        ...
        WILDSDatasetEnv(
            ...
            env_config = WILDS_multiple_to_single(OmegaConf.create(celebA_config["train_config"]))
        ),
    ]
)
...
final_ds = PosteriorAgreementDatasetPairing(dataset=ds, strategy="label_nn", feature_extractor=feature_extractor)
